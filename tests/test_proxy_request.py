...
def test_get_session_without_sending_the_protocol_as_http(proxy_request, protocol) -> NoReturn:
    session: requests.Session = proxy_request._get_session(host_and_port='0.0.0.0:80', protocol=protocol)
    assert session.proxies == {"http": f'{protocol}://0.0.0.0:80', "https": f'{protocol}://0.0.0.0:80'}


def test_get_session_sending_the_protocol_as_http(proxy_request) -> NoReturn:
    session: requests.Session = proxy_request._get_session(host_and_port='0.0.0.0:80', protocol='http_https')
