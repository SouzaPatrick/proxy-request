...
class ProxyRequest:
    def __init__(self, timeout: int = 10, destination_host: str = '', destination_host_port: int = 80) -> None:
        '''
    ...
    def _get_session(self, host_and_port: str, protocol: str) -> requests.Session:
        ...
        if protocol != 'http_https':
            ...
            session.proxies = {"http": session_host_and_port, "https": session_host_and_port}
        else:
            session.proxies = {"https": host_and_port, "http": host_and_port}

        return session

    def request(self, host_and_port: str, protocol: str = 'http_https') -> Optional[requests.Response]:
        '''
        :param host_and_port: HOST_IP:PORT of the proxy that will be used in the request attempt
        :param protocol: http_https | sock4 | sock5
        :return: Response from the request
        ...
        if self.destination_host:
            ...
            response: Optional[requests.Response] = session.get(self.destination_host, timeout=self.timeout)
        else:
